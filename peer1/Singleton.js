...
module.exports = {
    ...
    //--------------------------
    //getKeyID: takes the image name and returns 20 bytes Hex number
    //--------------------------
    getKeyID: function (imgName) {
        var crypto = require('crypto')
        var sha1 = crypto.createHash('sha1')
        sha1.update(imgName)
        return sha1.digest('hex')
     ...
     //--------------------------
    //getrandom port > 3000 && < 5000
    //--------------------------
    getImgPort: function() {
        var weight = Math.floor(Math.random() * (2001) + 3000);
        return  weight;
    },
